...
from ..base import Graph, Group, Node, DagInfo
...
class Namer:
    ...
    def __init__(self, atlas: nx.DiGraph):
        self.name_counter: collections.defaultdict[
            str, int
    ...
    def name_node(self,id:int):
        if id not in self.atlas.nodes:
            ...
            title = suggestion if self.name_counter > 1 else f"{suggestion}_{self.name_counter[suggestion]}"
        ...
        return title, NodeOutput(
            dependencies=deps_name,
...
class GraphProcessor:
    ...
    def from_preset(self, preset: Preset):
        ...
        self.format = preset.format_output_settings.format
