...
class NodeOutput(BaseModel):
    ...
    def from_base_node(cls,node:Node):
        return cls(properties = node.properties,
...
class GroupOutput(BaseModel):
    ...
    def from_base_node_generator(cls, gen: tp.Iterator[str,Node]):
        return cls(
            nodes =  {title: node for title,node in gen}
        )
...
class Namer:
    def __init__(self, preset: Preset):
        self.default_name_counter: collections.defaultdict[str,int] = collections.defaultdict(lambda: 0)
    ...
    def __call__(self, obj: Node | Group | Graph | GroupedGraph):
        ...
        if isinstance(obj, Node):
            title = self.name(obj)
            return {
                title: NodeOutput.from_base_node(obj)
            }
    
        elif isinstance(obj, Group):
            title = self.name(obj)
            return {
                title: GroupOutput.from_base_node_generator(self(node) for node in obj.nodes)
            }
